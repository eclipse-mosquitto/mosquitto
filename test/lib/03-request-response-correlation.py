...
def do_test(testdir):
    ...
    env = dict(os.environ)
    client1 = mosq_test.start_client(filename=f"{testdir}-03-request-response-correlation-1.log", cmd=[testdir / mosq_test.get_build_type() / "03-request-response-correlation-1.exe", str(port)])
