...
def do_test(testdir):
    ...
    client1 = mosq_test.start_client(filename=f"{testdir}-03-request-response-1.log", cmd=[Path(mosq_test.get_build_root(), 'test', 'lib', testdir, mosq_test.get_build_type(), '03-request-response-1.exe'), str(port)])
