...
do_test(Path("c", mosq_test.get_build_type(), "08-ssl-connect-cert-auth.exe"))
do_test(Path("c", mosq_test.get_build_type(), "08-ssl-connect-cert-auth-custom-ssl-ctx.exe"))
do_test(Path("c", mosq_test.get_build_type(), "08-ssl-connect-cert-auth-custom-ssl-ctx-default.exe"))
if mosq_test.check_features(["WITH_LIB_CPP"]):
    do_test(Path("cpp", mosq_test.get_build_type(), "08-ssl-connect-cert-auth.exe"))
    do_test(Path("cpp", mosq_test.get_build_type(), "08-ssl-connect-cert-auth-custom-ssl-ctx.exe"))
    do_test(Path("cpp", mosq_test.get_build_type(), "08-ssl-connect-cert-auth-custom-ssl-ctx-default.exe"))
