...
def do_test(testdir, client_cmd):
    rc = 1
    ...
    client_args = [Path(mosq_test.get_build_root(), "test", "lib", testdir, mosq_test.get_build_type(), client_cmd), str(port)]
    client = mosq_test.start_client(filename=client_cmd, cmd=client_args)
...
do_test('c', '08-ssl-connect-cert-auth-enc.exe')
do_test('cpp', '08-ssl-connect-cert-auth-enc.exe')
